...
class NCF(nn.Module):
    ...
    def forward(self, users, items):
        u_emb = self.user_emb(users).squeeze(1)
        i_emb = self.item_emb(items).squeeze(1)
        x = torch.cat([u_emb, i_emb], dim=1)
        return self.mlp(x).squeeze()

# TRAINING FUNCTION 
def train_model(model, dataloader, epochs=100, lr=0.01, verbose=True):
    model.train()
...
# TIME-BASED USER-ITEM TIMESTAMPS
...
#  EVALUATION ITEM SIM
def evaluate_ncf_with_similarity(model, test_interactions, train_set, item_similarity_matrix, ks=[5, 10, 20], alpha_item=0.5):
    model.eval()
    ...
    with torch.no_grad():
        for u, true_i in test_interactions:
            ...
            ncf_range = np.max(ncf_scores) - np.min(ncf_scores)
            if ncf_range > 1e-10:
                ncf_scores = (ncf_scores - np.min(ncf_scores)) / ncf_range
            
            sim_range = np.max(item_sim_scores) - np.min(item_sim_scores)
            if sim_range > 1e-10:
                item_sim_scores = (item_sim_scores - np.min(item_sim_scores)) / sim_range

            fused_scores = (1 - alpha_item) * ncf_scores + alpha_item * item_sim_scores
...
def community_time_recommendations(model, user_id, item_ids, user_item_timestamps, 
    ...
    """
    Apply community-based time similarity to NCF scores for recommendation using a weighted average approach.
    """
    alpha_ncf = 1.0 - alpha_time
    ...
    fused_scores = alpha_ncf * ncf_scores + alpha_time * time_scores
    
    return fused_scores

#  EVALUATION WITH COMMUNITY TIME 
def evaluate_ncf_with_community_time(model, test_interactions, train_interactions, 
                                      user_item_timestamps, user_community, item_community,
                                      ks=[5, 10, 20], alpha_time=0.5):
    """
    Evaluate NCF model with community-based time similarity
    """
    model.eval()
    metrics = {k: {'hit': 0, 'ndcg': 0.0, 'recall': 0.0} for k in ks}
    user_interactions = defaultdict(set)
...
if __name__ == "__main__":
    ...
    #  ITEM SIMILARITY 
    print("\n" + "="*40)
    print("ABLATION 1: NCF WITH ITEM SIMILARITY")
    print("="*40 + "\n")
    ...
    for alpha_item in tqdm(alpha_values, desc="Evaluating alpha_item values"):
        ...
        for run in range(1, num_runs + 1):
            ...
            run_results = evaluate_ncf_with_similarity(
                model, 
                test_interactions, 
                train_interactions, 
    ...
    csv_file_item_sim = f"ncf_alpha_item_similarity_amazon_health_household.csv"
    ...
    print("\n" + "="*40)
    print("ABLATION 2: NCF WITH TIME-BASED COMMUNITY SIMILARITY")
    print("="*40 + "\n")
    ...
    for alpha_time in tqdm(alpha_values, desc="Evaluating alpha_time values"):
        ...
        for run in range(1, num_runs + 1):
            ...
            run_results = evaluate_ncf_with_community_time(
                model, 
                test_interactions, 
                train_interactions, 
                user_item_timestamps, 
    ...
    time_sim_df = pd.DataFrame(time_sim_output_rows)
    csv_file_time_sim = f"ncf_alpha_time_similarity_amazon_health_household.csv"
    time_sim_df.to_csv(csv_file_time_sim, index=False)
    
    elapsed_time = time.time() - start_time
    print(f"Time Similarity Evaluation complete in {elapsed_time/60:.2f} minutes. Results saved to '{csv_file_time_sim}'")
    print("\nPreview of Time Similarity results:")
